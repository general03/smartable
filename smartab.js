...
(function() {
	...
	smartab.prototype.sortTable = function(column){
		...
		var rows = this.table.rows, arr = new Array(), arrayHidden = new Array(), i, j, cells, cellLength, rowLength = rows.length;
	    ...
	    for(i = 0; i < arr.length; i++){
	    	if(arr[i] != undefined && arr[i].length > 0)
	    		arr[i] = "<td>"+arr[i].filter(Boolean).join("</td><td>")+"</td>";
	    }
		...
		content.innerHTML = '<tr>'+arr.filter(Boolean).join('</tr><tr>')+'</tr>';
